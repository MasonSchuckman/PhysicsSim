...
public class GameObject implements Cloneable
{
	...
	protected  Vector3d velocity2;
	protected  Vector3d position2;
	protected  int r1 = ThreadLocalRandom.current().nextInt(0, 253),r2 = ThreadLocalRandom.current().nextInt(0, 253),r3 = ThreadLocalRandom.current().nextInt(0, 253);    
	public   int identifier = ThreadLocalRandom.current().nextInt(0, 100);
	public   float mass,bounciness=1,drag=1;
	protected  int w=10;
	protected int h=10;
	protected int z=10;
	...
	protected  int w2=5,h2=5;    
	public Sphere ball;
	public String type;    
	public ArrayList<Integer> recentlyHit = new ArrayList<Integer>();

	public GameObject(float Vi,float angle,int currentX,int currentY,int currentZ)
	{
	...
	public GameObject(int currentX,int currentY,int currentZ)
	{
		fxColor= javafx.scene.paint.Color.rgb(230, 230, 100, 1);
	...
	public GameObject(float Vi,float angle,float mass,int currentX,int currentY,int currentZ, int w,float bounciness,float drag)
	{
		fxColor= javafx.scene.paint.Color.rgb(230, 230, 100, 1);
		if(drag==1||mass>1000)fxColor= javafx.scene.paint.Color.rgb(60, 60, 200, 1);
		if(mass<0)fxColor= javafx.scene.paint.Color.rgb(200, 60, 60, 1);
	...
	//how many triangles make up the sphere. The higher the number, the higher the "resoultion" of the sphere.
	int divisions=10;
	
	public void makeSphere() {
		//ball=new Sphere(w,divisions);
		ball=new Sphere(w);
	}
	...
	public GameObject clone() throws
